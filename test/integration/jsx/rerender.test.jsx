...
describe('jsx, rerendering', () => {
  describe('basic tags', () => {
    ...
    xit('updates attributes', () => {
      ...
      assertEquals(
        domToString(dom),
        '<h1 class="text-bold highlighted">Hello World</h1>',
      );
    });
  ...
  describe('layered abstract delegates', () => {
    xit('replaces correctly when changing types', () => {
      const document = createTestDom();

      const Template = ({ name, className }) => (
        <h1 class={className}>Hello {name}</h1>
      );
      const BoundTemplate = bind(Template, (state) => ({ name: state.name }));
      const ReboundTemplate = bind(BoundTemplate, (state) => {
        let className = 'text-bold';
      ...
      const template = <ReboundTemplate />;

      let [dom] = template.render({ document, state: { name: 'Guest' } });
      assertEquals(domToString(dom), '<h1 class="text-bold">Hello Guest</h1>');

      [dom] = template.rerender({
        document,
        state: { name: 'Kane', highlighted: true },
      });
      assertEquals(
        domToString(dom),
        '<h1 class="text-bold highlighted">Hello Kane</h1>',
      );
    });
  });
});
