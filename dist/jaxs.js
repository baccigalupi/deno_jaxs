...
const queryParams = (queryString) => {
  return queryString.replace(/^\?/, '').split('&').reduce(
    (aggregate, pairString) => {
...
const locationChanged = (state, locationData) => {
  ...
  return locationData.hash !== state.hash || locationData.path !== state.path ||
...
const __default1 = createReducer('updateLocation').initialState(() =>
  dataFromLocation()
...
function dew2() {
  ...
  function createStore(reducer, preloadedState, enhancer) {
    ...
    if (
      typeof preloadedState === 'function' && typeof enhancer === 'undefined'
    ) {
  ...
  function getUndefinedStateErrorMessage(key, action) {
    ...
    return 'Given ' + actionDescription + ', reducer "' + key +
      '" returned undefined. ' +
      ...
      if (
        ...
        throw new Error(
          'Reducer "' + key +
            ...
            ('Don\'t try to handle ' + ActionTypes.INIT +
...
const bind1 = (Template, viewModel) => {
...
export { jsx1 as jsx };
export { __default4 as createApp };
export { bind1 as bind };
export { router1 as router };
